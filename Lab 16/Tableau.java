...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        Integer returnedValue = addToRow(smallest, value);

        Cell curr = smallest;

        while (returnedValue != null) {
            if (curr.south == null) {
                Cell newCell = new Cell(returnedValue);
            ...
            returnedValue = addToRow(curr, returnedValue);
        }
    }
    ...
    protected Integer addToRow(Cell curr, int value) {
        ...
        if (curr.east == null && value > curr.value) {
            Cell newCell = new Cell(value);

            newCell.west = curr;
            curr.east = newCell;

            Cell northCell = (curr.north != null) ? curr.north.east : null;
            Cell southCell = (curr.south != null) ? curr.south.east : null;

            if (northCell != null) {
                newCell.north = northCell;
                northCell.south = newCell;
            }

            if (southCell != null) {
                newCell.south = southCell;
                southCell.north = newCell;
            }
    ...
    protected void print(Function<Cell,Integer> f) {
        for (Cell i = smallest; i != null; i = i.south) {
            for (Cell j = i; j != null; j = j.east) {
